...
- AI Summary: Vercel serverless function (`api/summary.ts`) calls Gemini 2.0 Flash with answer pairs + followupPrompt + aiHint. Client fetches on results page when both participants have answered; falls back to local insights if API unavailable. `GEMINI_API_KEY` env var needed on Vercel production.
